...
gatkVcf <- "data/VCF-GATK-HC-annot.all.merged.txt"
lofreqVcf <- "data/VCF-LoFreq-annot.all.merged.txt"
```
...
# LoFreq VCF file 
lofreqVariantCalls <- read_delim(file = lofreqVcf,
                                 delim = "\t",
                                 col_names = TRUE
                                 )
...
# Low-Complexity Region filter
lcrVariantCalls <- read_delim(file = "../data/variantCalls.lcrfilter.txt",
                              delim = "\t",
                              col_names = TRUE
                              )
...
# Hardy-Weinberg filter
hwVariantCalls <- read_delim(file = "../data/variantCalls.hwfilter.txt",
                             delim = "\t",
                             col_names = TRUE
                             )
...
# Excess Coverage filter
ecVariantCalls <- read_delim(file = "../data/variantCalls.ecfilter.txt",
                            delim = "\t",
                            col_names = TRUE
                            )
...
# Segmental Duplication filter
sdVariantCalls <- read_delim(file = "../data/variantCalls.sdfilter.txt",
                             delim = "\t",
                             col_names = TRUE
                             )
...
# Strict Mask filter
smVariantCalls <- read_delim(file = "../data/variantCalls.smfilter.txt",
                             delim = "\t",
                             col_names = TRUE
                             )
...
lollipopMaf <- readMAF(maf.file = "../data/lollipopPlot.maf", if.parse.aa.pos = FALSE)
...
ggplot(ch_udVsAll, aes(x = ageBins, y = percentageVsAll, group = 1)) +
  ...
  annotate(geom = "text",
           x = seq(1,12, 1),
           y = 1.25,
           label = paste0(ch_udAgeBins$binCount, "/", allSamplesAgeBinned$binCount),
           size = 4,
           )
